...
def check_report_status(session, status_url, bearer_token):
    while True:
...
def download_report(session, download_url, bearer_token, output_file):
    response = session.get(
        download_url,
        headers={
            'Authorization': f'Bearer {bearer_token}',
...
def main():
    ...
    status_url = download_url.replace('/download', '')
    ...
    check_report_status(session, status_url, bearer_token)
    download_report(session, download_url, bearer_token, 'license-report.zip')
